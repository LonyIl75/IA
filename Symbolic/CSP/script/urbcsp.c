...
int main(int argc, char* argv[])
{
  ...
  if (argc != 7)
    {
      printf("\n");
      printf("\tusage: urbcsp nbVariables tailleDomaine nbConstraints nbTuples Seed nbInstances\n");
...
int MakeURBCSP(int N, int D, int C, int T, long *Seed)
{
  ...
  if (N < 2)
    {
      //comment :printf("MakeURBCSP: ***Illegal value for N: %d\n", N);
      return 0;
    }
  if (D < 2)
    {
      //comment :printf("MakeURBCSP: ***Illegal value for D: %d\n", D);
      return 0;
    }
  if (C < 0 || C > N * (N - 1) / 2)
    {
      //comment :printf("MakeURBCSP: ***Illegal value for C: %d\n", C);
      return 0;
    }
  if (T < 1 || T > ((D * D) - 1))
    {
      //comment :printf("MakeURBCSP: ***Illegal value for T: %d\n", T);
      return 0;
    }
  ...
  for (var1=0; var1<(N-1); var1++)
    for (var2=var1+1; var2<N; var2++) {
      CTarray[i].p1 = var1;
      CTarray[i].p2 = var2;
      //comment : printf("On met dans %d :  %d et %d \n",i,var1,var2);
      i++;
    }
  //comment :printf("%d tuples de variables générés\n",i);
  ...
  for (c=0; c<C; ++c)
    ...
    //comment : printf("***On choisit le %d qui a %d et %d\n",r, selectedCT.p1, selectedCT.p2);
