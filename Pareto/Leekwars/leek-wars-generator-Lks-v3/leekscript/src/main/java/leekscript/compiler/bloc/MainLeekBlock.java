...
public class MainLeekBlock extends AbstractLeekBlock {
	...
	public void writeJavaCode(JavaWriter writer, String className, String AIClass) {
		...
		writer.addLine("private Object[] arrays_json = null;");
		...
		//writer.addLine("System.out.println(\"FIGHT : \" + fight.getConfig().toString())+\" GENID \" + fight.getConfig().getGenID() +\" FIGHT ID \" + fight.getConfig().getFigthID())  ;");
		//writer.addLine("if( fight.getConfig().getGenID() > 0 ){ g_gene = readAdn.getAdnFromFile(  fight.getConfig().getSettingFilename() ,fight.getConfig().getGenID() ,fight.getConfig().getFigthID());g_init_gene = true;}");
		...
		super.writeJavaCode(this, writer);//ICI GENE
