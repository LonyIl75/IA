	...
	public static AI compile(AIFile file, boolean useClassCache) throws LeekScriptException, LeekCompilerException {
		...
		if (useClassCache) {
			...
			try {
				...
				javaOutput.write(file.getCompiledCode().getJavaCode().getBytes(StandardCharsets.UTF_8));//WRITE JAVA
				javaOutput.close();
			} catch (IOException e) {
        ...
        // Récupération du fichier manager

        // Création d'un diagnostic listener pour afficher les messages d'erreur
		...
		for (var compiledClass : fileManager.getCompiled().values()) {
			...
			if (useClassCache) { // Save bytecode
				try {
					var classFile = new FileOutputStream(Paths.get(IA_PATH, compiledClass.getName() + ".class").toFile());// WRITE CLASS JAVA 
