...
public class EntityAI extends AI {
	...
	public static AIFile resolve(Generator generator, EntityInfo entityInfo, Entity entity) {
		...
		try {
			if (entityInfo.ai_path != null) {
				System.out.println("Resolve AI " + entityInfo.ai_path + "...");
				...
				System.out.println("Resolve AI " + entityInfo.ai + "...");
	...
	public String getIDFightSetting(){
		return fight.getConfig().getGenFight();
	}
	public Object getAttributFromFile(String attribut_key ){
		return readAdn.getFromFile(  fight.getConfig().getSettingFilename() ,getIDFightSetting() ,  String.valueOf(getEntity().getId()) , attribut_key );
	}
	public Object getAdnFromFile(){ //?String
		return  getAttributFromFile("adn" );
	}
	public boolean isAvaibleFromFile(){
		return fight.getConfig().getGenID() > 0 ;
	}
	...
	public void runTurn(int turn ) {
		...
		try {
			...
			if (turn == 1) {
				staticInit();
				//TimeUnit.SECONDS.sleep((int)(fight.getConfig().getFigthID()+1)/2);//TO FIX !!!
				setConfig();
				//TimeUnit.SECONDS.sleep(fight.getConfig().getFigthID());
			}
