...
public class Fight {
	...
	public Fight(Generator generator, FightListener listener , GenConfig config) { 
	...
	public HashMap<Integer , ArrayList<double[][]>> mhistory = new HashMap<Integer , ArrayList<double[][]>>();

	public void addHistory(int entity_id){
		double[][] tmp_stats = getEntity(entity_id).getStats();
		if(!mhistory.containsKey(entity_id))
			mhistory.put(entity_id, new ArrayList<double[][]>());
		mhistory.get(entity_id).add(tmp_stats);

	}

	public  HashMap<Integer , ArrayList<double[][]>> getHistory(){
		return mhistory;
	}
	...
	public int useWeapon(Entity launcher, Cell target) {
		...
		//System.out.println("UTILISE WEAPON");
	...
	public int useChip(Entity caster, Cell target, Chip template) {
		...
		//System.out.println("UTILISE CHIP");
