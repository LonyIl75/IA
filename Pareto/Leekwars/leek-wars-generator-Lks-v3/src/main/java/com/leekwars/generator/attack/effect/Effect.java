...
public abstract class Effect {
	...
	public static int createEffect(Fight fight, int id, int turns, double aoe, double value1, double value2, boolean critical, Entity target, Entity caster, Attack attack, double jet, boolean stackable, int previousEffectTotalValue, int targetCount, int propagate, int modifiers) {
		...
		//System.out.println("J'ai creer effet");
		//ActionAddEffect.createEffect(fight.getActions(), id , attack.getItemId(),  caster, target, fight.getActions().getEffectId() ,value1 + value2 * jet, turns,  stackable , modifiers);
		...
		ActionAddEffect.createEffect(fight.getActions(), attack.getType() , attack.getItemId(),  caster, target,id, (int) (value1 + value2 * jet), turns,  stackable , modifiers);
	...
	public void addLog(Fight fight) {
		...
		logID = ActionAddEffect.createEffect(fight.getActions(), attack == null ? Attack.TYPE_CHIP : attack.getType(), attack == null ? 0 : attack.getItemId(), caster, target, id, value, turns, false, modifiers);
	}
